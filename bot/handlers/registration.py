...
async def confirm_registration(
    ...
    try:
        ...
        existing_user = await get_user_by_telegram_id(session, query.from_user.id)
        ...
        # Если пользователь уже существовал - просто обновляем данные без генерации плана
        if not is_new_user:
            update_message = (
                "✅ <b>Данные обновлены!</b>\n\n"
                "Я сохранил изменения в вашем профиле. "
        ...
        # Для новых пользователей - показываем благодарность и генерируем план
        thanks_message = (
