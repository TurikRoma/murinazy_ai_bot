"""
Тестовый скрипт для проверки генерации НЕДЕЛЬНОЙ программы тренировок
...
TEST_USER_PROFILE = {
    ...
    "fitness_level": "beginner", # beginner (стаж < 1 года) | advanced (стаж > 1 года)
    "workout_frequency": 3,      # 2 | 3 | 5
    "equipment_type": "gym",     # gym | bodyweight
    "session_duration": 60       # Желаемая длительность в минутах
}

TEST_TRAINING_CONTEXT = {
    "current_training_week": 1 # Номер недели в цикле (от 1 до 6 для новичков, от 1 до 3 для продвинутых)
}
...
2.  **Построение Сплита (распределение по дням):**
...
4.  **Правило Времени Отдыха (rest_seconds):** Ты ОБЯЗАН устанавливать значение `rest_seconds` строго в зависимости от цели текущей недели, определенной в правиле №1.
...
5.  **Прогрессивная Нагрузка:**
    *   Прогрессия нагрузки СТРОГО определяется параметром `current_training_week` и выбранной моделью периодизации.
...
{{
  ...
  "workout_plan": [
    ...
    // ВНИМАНИЕ: Для `fitness_level: beginner` и сплита `Full Body`,
    // этот массив `workout_plan` ВСЕГДА ДОЛЖЕН СОДЕРЖАТЬ ТОЛЬКО ОДИН ЭЛЕМЕНТ.
  ]
}}
"""
...
def load_and_flatten_exercises(user_equipment_type: str) -> list:
    """
    Загружает упражнения из вложенного JSON, преобразует их в плоский список
    объектов и фильтрует по типу оборудования пользователя.
    """
    try:
        ...
        print("❌ ОШИБКА: Неверный формат JSON в файле 'exercises_by_muscle_and_type.json'.")
        return []
        
    # Словарь для сопоставления ключей из JSON с нашими ключами
    equipment_map = {
        "Зал": "gym",
        "Свой вес": "bodyweight"
    }
    ...
    flattened_list = []
    # Проходим по всем группам мышц (e.g., "Грудь", "Спина")
    for muscle, equipment_groups in nested_exercises.items():
        # Проходим по типам оборудования внутри группы (e.g., "Зал", "Свой вес")
        for equipment_name, exercises in equipment_groups.items():
            equipment_key = equipment_map.get(equipment_name)
            if not equipment_key:
                continue # Пропускаем неизвестные типы оборудования
            
            # Создаем объекты для каждого упражнения в списке
    ...
    # Фильтруем итоговый список по типу оборудования пользователя
    final_filtered_list = [
...
async def generate_workout_week(
    ...
    """
    Формирует полный промпт и генерирует НЕДЕЛЬНУЮ программу тренировок с помощью LLM.
    """
    prompt_data = {
    ...
    # print(final_prompt) # Раскомментируйте для полной отладки промпта
...
async def main():
    ...
    if not exercises:
        print("Не удалось загрузить упражнения. Завершение работы.")
        return
    ...
    try:
        ...
        print("\n" + "="*80)
        print(f"❌ КРИТИЧЕСКАЯ ОШИБКА: LLM вернула невалидный JSON.")
        print("="*80)
